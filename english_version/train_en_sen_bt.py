...
# should add x = utils.split_document(x[0])
X_train = []
...
cut_val, sen_len = utils.get_sen_stat([k[0] for k in X_train] + [k[0] for k in X_test], cut_thres = 0.95)
...
tokenizer, estimator = embedding_bert.prepare_bert(bert_vocab_file, bert_config_file, init_checkpoint, sen_len, select_layers, graph_file)
...
utils.get_sen_batch_seq(np.squeeze(c, axis = 1), pad_token = 0)
...
with tf.name_scope('inputs'):
    ...
    X_emb = tf.placeholder(shape=(None, sen_len, 3072), dtype=tf.float32, name='inputs')
...
utils.split_sentence(X_batch[0][0])
