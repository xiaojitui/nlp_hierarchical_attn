...
# refine , if necessary
len(X_train), len(X_train_doc), len(X_test), len(X_test_doc)
...
doc_cut_val, sen_cut_val, _, _ = utils.get_doc_stat(X_train_doc + X_test_doc, doc_cut_thres = 0.95, sen_cut_thres = 0.95)
...
with tf.name_scope('logits'):
    optimizer, logits, cost, accuracy, Y_proba = model.clf_train_op(final_outputs_drop, y, ac_fn = tf.nn.relu, 
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        for _ in range(n_batch):
            ...
            X_batch_emb = utils.token_docs(X_batch, doc_len, sen_len, word_to_idx)
            doc_seq_len, sen_seq_len = utils.get_doc_batch_seq(X_batch_emb, pad_token = 400001)
            ...
            _, loss, acc, sen_alpha, doc_alpha = sess.run([optimizer, cost, accuracy, sen_atten_w, doc_atten_w],
                                          feed_dict={X: X_batch_emb, y: y_batch, 
...
with tf.Session() as sess:
    ...
    X_batch, y_batch = utils.get_batch_data(X_test_doc, y_test, 7)
    X_batch_emb = utils.token_docs(X_batch, doc_len, sen_len, word_to_idx)
    doc_seq_len, sen_seq_len = utils.get_doc_batch_seq(X_batch_emb, pad_token = 400001)
    #batch_seq_len = np.array([list(x).index(0) + 1 for x in X_batch])
    sen_alpha, doc_alpha, y_proba = sess.run([sen_atten_w, doc_atten_w, Y_proba], feed_dict={X: X_batch_emb, 
                                                                                             doc_seq_length: doc_seq_len, 
...
sen_alpha = sen_alpha.reshape(-1, doc_len, sen_len)
sen_alpha.shape, doc_alpha.shape
...
for i in range(len(s)):
    print(s[i], round(doc_alpha[doc_idx][i], 3), end = ';  ')

    
sen_idx = 0
alpha = sen_alpha[doc_idx, :, :]
fig = plt.figure(figsize=(20, 5)) 
ax = fig.add_subplot(1, 1, 1)
cax = ax.matshow(alpha, cmap='viridis') #alpha[0, :][:, np.newaxis].T
fig.colorbar(cax, orientation = 'horizontal', shrink = 0.2) 
ax.set_xticks(np.arange(sen_len))
ax.tick_params(length=0)
#ax.set_xticklabels([k for k in s[sen_idx].split()], fontdict={'fontsize': 12}, rotation=40, ha='left')
#ax.set_yticklabels([''])
...
fig = plt.figure(figsize=(20, 5)) 
ax = fig.add_subplot(1, 1, 1)
cax = ax.matshow(alpha[sen_idx, :][:, np.newaxis].T, cmap='viridis') #alpha[0, :][:, np.newaxis].T
fig.colorbar(cax, orientation = 'horizontal', shrink = 0.2) 
ax.set_xticks(np.arange(sen_len))
ax.tick_params(length=0)
ax.set_xticklabels([k for k in s[sen_idx].split()], fontdict={'fontsize': 12}, rotation=40, ha='left')
