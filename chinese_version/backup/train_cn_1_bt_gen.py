...
train_ds = get_train_data(train_data, batch_size)
iter_op = train_ds.make_initializable_iterator()
X_emb, y, doc_seq_length, sen_seq_length = iter_op.get_next()
#doc_seq_length = doc_seq_length.flatten()
#sen_seq_length = sen_seq_length.flatten()
#doc_seq_length = tf.reshape(doc_seq_length, [-1])
#sen_seq_length = tf.reshape(sen_seq_length, [-1])
...
with tf.name_scope('sen_rnn'):
    ...
    sen_rnn_outputs, sen_rnn_states = rnn_layer.bi_rnn(X_embed_reshape, n_hidden = n_hidden, seq_len = tf.reshape(sen_seq_length, [-1]), n_layer = n_layer, 
...
with tf.name_scope('sen_attn'):
    sen_atten_out, sen_atten_w = attn_layer.atten_layer_project(sen_rnn_outputs, atten_size, n_layer = n_layer, l2reg = l2reg, 
                                                           seq_len = tf.reshape(sen_seq_length, [-1]), use_mask = use_mask, sen_CLS = sen_CLS, 
...
with tf.name_scope('doc_rnn'):
    ...
    doc_rnn_outputs, doc_rnn_states = rnn_layer.bi_rnn(doc_inputs, n_hidden = n_hidden, seq_len = tf.reshape(doc_seq_length, [-1]), n_layer = n_layer, 
...
with tf.name_scope('doc_attn'):
    doc_atten_out, doc_atten_w = attn_layer.atten_layer_project(doc_rnn_outputs, atten_size, n_layer = n_layer, l2reg = l2reg, 
                                                           seq_len = tf.reshape(doc_seq_length, [-1]), use_mask = use_mask, scope = 'doc_attn_block')
    # Dropout
...
with tf.Session(config=config) as sess:
    ...
    for epoch in range(epochs):
        ...
        for n_iter in tqdm(range(n_batch), total = n_batch):
            ...
            loss_train += loss
            try:
                rank_val = spearmanr(np.squeeze(y_pred_val), y_batch, nan_policy='omit')[0]
            except:
                rank_val = 0
                
            if n_iter % 20 == 0:
                tqdm.write(str(rank_val)) #, flush = True
            rank_train += rank_val
        #acc_train /= n_batch
        loss_train /= n_batch
        rank_train /= n_batch
