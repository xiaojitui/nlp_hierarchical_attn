...
with tf.device('/gpu:2'):
    with tf.name_scope('final_out'):
        ...
        final_outputs_drop = tf.layers.dropout(final_outputs, rate = 1-0.5, training = is_training)

    with tf.name_scope('logits'):
        optimizer, logits, cost = reg_train_op(final_outputs_drop, y, is_train = is_training, 
                                               ac_fn = tf.nn.tanh, lr = 1e-3, l2reg = 0.001, n_output = 1)
...
with tf.Session(config=tf.ConfigProto(log_device_placement=True, allow_soft_placement=True)) as sess:
    ...
    #saver.restore(sess, './saved_model/best_model_ws')
    min_loss = np.inf
    ...
    for epoch in range(epochs + 1):
        ...
        if loss_test < min_loss:
            saver.save(sess, './saved_model/best_model_ws')
            min_loss = loss_test

    saver.save(sess,  './saved_model/final_model_ws')
