...
pos_data = pd.read_excel(os.path.join(path, 'pos.xls'), header = None)
neg_data = pd.read_excel(os.path.join(path, 'neg.xls'), header = None)
...
pos_data = pos_data.values
neg_data = neg_data.values
...
train_size = int((10677+10428)*0.85)
test_size = int((10677+10428)*0.15)
...
pos_train_idx = np.random.choice(10677, 17940//2, replace=False)
pos_train = [pos_data_clean[k] for k in pos_train_idx]
pos_test = [pos_data_clean[k] for k in range(10677) if k not in pos_train_idx]


neg_train_idx = np.random.choice(10428, 17940//2, replace=False)
neg_train = [neg_data_clean[k] for k in neg_train_idx]
neg_test = [neg_data_clean[k] for k in range(10428) if k not in neg_train_idx]
...
graph_file = '../bert/tmp_ch/graph_doc_dt_phrase' # where to put temporary graph
...
savepath = '/ids/datasets/CNN_datasets/saved_weights_n/best_model_1.ckpt'
finalpath = '/ids/datasets/CNN_datasets/saved_weights_n/final_model_1.ckpt'
